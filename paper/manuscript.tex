...
(held-out source subjects), never on the target. It is therefore a montage-level competence
score needing no target eye-tracker: an arbitrary no-eye-tracker target inherits the score of
its matched source montage, and is gated \emph{inconclusive} when no matched source can
reconstruct gaze. Thresholds are derived
...
\texttt{eegmmidb}, and Brain Invaders); \emph{Code availability:} the auditor, simulator, and analysis scripts (R, CPU-only) and all
result tables will be made publicly available upon publication. This study used only publicly available,
